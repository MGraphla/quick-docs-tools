...
const WordToPdfPage = () => {
  ...
  const convertToPdf = async () => {
    ...
    try {
      ...
      const steps = [
        { message: "Analyzing Word documents...", progress: 15 },
      ...
      toast.success(`Successfully converted ${files.length} Word document${files.length > 1 ? 's' : ''} to PDF`);
    ...
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <Button
          ...
          className="flex-1 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white py-3 text-lg font-semibold"
          size="lg"
        >
          {converting ? (
            <>
      ...
      {files.length === 0 && (
        <Alert>
          ...
          <AlertDescription>
            <strong>How to convert Word to PDF:</strong> Upload Word documents (.doc or .docx), configure your PDF quality and formatting options, then click "Convert to PDF" to create professional PDF files.
          </AlertDescription>
        </Alert>
      )}
    </div>
